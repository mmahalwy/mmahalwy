...
const TwitterAuthButton = ({
  ...
  const handleAuth = () => {
    const q = queryString.stringify({ source_url: window.location.origin, user_id: userId });
...
```tsx
export const authenticate = ({ provider, url, tab = false, cb }: AuthenticateArg) => {
...
const Auth = () => {
  ...
  return <div>{jwt ? <Title1>Loading...</Title1> : <Title1>Authentication failed</Title1>}</div>;
};
